...
window.addEventListener('mousemove', (e) => { mouse.x = e.clientX; mouse.y = e.clientY; if (socket.readyState === WebSocket.OPEN) socket.send(JSON.stringify({ type: 'move', mouseX: mouse.x, mouseY: mouse.y })); });
window.addEventListener('mousedown', (e) => { let em = e.button === 0 ? 'angry' : (e.button === 2 ? 'sad' : 'normal'); if (socket.readyState === WebSocket.OPEN) socket.send(JSON.stringify({ type: 'changeEmotion', emotion: em })); });
window.addEventListener('mouseup', () => { if (socket.readyState === WebSocket.OPEN) socket.send(JSON.stringify({ type: 'changeEmotion', emotion: 'normal' })); });
window.addEventListener('contextmenu', (e) => e.preventDefault());
socket.onmessage = (e) => {
    let data = JSON.parse(e.data);
    if (data.type === 'init') myId = data.id;
    if (data.type === 'update') { players = data.players; mobs = data.mobs; if (players[myId]) { score = players[myId].score; if (scoreElement) scoreElement.innerText = "Очки: " + score; } }
};
...
function gameLoop() {
    ...
    mobs.forEach((mob) => {
        ...
        ctx.fillStyle = '#00ff00'; ctx.fillRect(mob.x - mob.radius, mob.y - mob.radius - 6, (mob.radius * 2) * (mob.hp / mob.maxHp), 4);
    });
